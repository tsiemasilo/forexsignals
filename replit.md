...
Changelog:
- August 15, 2025. DEDICATED TRIAL CREATION SYSTEM IMPLEMENTED: Completely rebuilt trial creation to permanently eliminate corruption. Removed problematic dropdown-based approach entirely and replaced with dedicated "Create 7-Day Trial" button system. Created new backend endpoint `/api/admin/users/:userId/create-trial` with `createFreshTrial` method that bypasses all previous corruption points. New system creates clean database entries with exactly 7-day duration, removes existing subscriptions before creating fresh trials, and provides detailed logging for monitoring. Admin panel now uses simple button interface instead of complex dropdown logic.

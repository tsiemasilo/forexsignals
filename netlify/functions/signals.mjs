...
export const handler = async (event, context) => {
  ...
  try {
    ...
    // Try to get trading signals from database, fallback to seed data if table doesn't exist
    let signals = [];
    ...
    try {
      ...
      console.log('Database signals table not found, using seed data:', dbError.message);
      signals = [];
    }

    // If no signals in database or table doesn't exist, return seed data
    if (signals.length === 0) {
